...
const ContentSection: React.FC<ContentSectionProps> = ({
  ...
  const renderTitle = () => {
    if (!highlightedWord) {
      return <h2 className="text-[#454545] text-[42px] font-bold max-md:text-[32px] max-sm:text-2xl text-center">{title}</h2>;
    }

    const parts = title.split(highlightedWord);
    return (
      <h2 className="text-[#454545] text-[42px] font-bold max-md:text-[32px] max-sm:text-2xl text-center">
        {parts[0]}
  ...
  const contentElement = (
    <div className="flex justify-center items-center gap-24 flex-1 w-full max-md:flex-col">
      {imageUrl && (
